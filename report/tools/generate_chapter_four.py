...
SOURCE = ROOT / "report" / "Chapter_Four_Source.md"
OUTPUT = ROOT / "report" / "Chapter_Four_Phishing_Digital_Medium_Detection_System.docx"
RENDER_DIR = ROOT / ".report-work" / "chapter-four-renders"
DIAGRAM_DIR = ROOT / "report" / "diagrams"

FIGURES = {
    "LANDING PAGE SCREENSHOT": {
        "number": "4.1", "placeholder": True,
        "note": "Replace with an actual browser capture showing the PhishGuard AI brand, email/SMS entry cards and Start Analysis action.",
    },
    "EMAIL FORM SCREENSHOT": {
        "number": "4.2", "placeholder": True,
        "note": "Replace with an actual browser capture of email mode showing sender, optional subject, message body and Analyze Message action.",
    },
    "SMS FORM SCREENSHOT": {
        "number": "4.3", "placeholder": True,
        "note": "Replace with an actual browser capture of SMS mode showing the SMS sender field, hidden subject and message body.",
    },
    "LOW-RISK EMAIL RESULT SCREENSHOT": {
        "number": "4.4", "placeholder": True,
        "note": "Replace with an actual Low Risk email result generated from a non-sensitive controlled message.",
    },
    "WARNING EMAIL RESULTS SCREENSHOT": {
        "number": "4.5", "placeholder": True,
        "note": "Replace with a two-panel composite of actual Suspicious and High Risk email results, with reasons and score visible.",
    },
    "SMS RESULT STATES SCREENSHOT": {
        "number": "4.6", "placeholder": True,
        "note": "Replace with a two-panel composite of actual safe and suspicious SMS results; retain masked sender display and hidden score.",
    },
    "VALIDATION AND ERROR STATES SCREENSHOT": {
        "number": "4.7", "placeholder": True,
        "note": "Replace with actual field-validation and controlled processing-error captures. Neither panel should display a risk verdict.",
    },
    "SMS METRIC SUMMARY": {
        "number": "4.8", "placeholder": False,
        "svg": DIAGRAM_DIR / "PhishGuard_AI_Chapter4_SMS_Metrics.svg",
        "landscape": False, "width": 6.05,
    },
    "SMS CONFUSION MATRIX": {
        "number": "4.9", "placeholder": False,
        "svg": DIAGRAM_DIR / "PhishGuard_AI_Chapter4_SMS_Confusion_Matrix.svg",
        "landscape": False, "width": 5.75,
    },
    "EMAIL CLASSIFIER RESULT PLACEHOLDER": {
        "number": "4.10", "placeholder": True,
        "note": "Insert only a confusion matrix and metric summary reproduced from verified held-out email data. Do not estimate missing values.",
    },
    "USABILITY RESULT PLACEHOLDER": {
        "number": "4.11", "placeholder": True,
        "note": "Insert only approved participant-study results, including sample, tasks, instrument and verified findings. No user study has yet been reported.",
    },
}

LANDSCAPE_TABLES = {"4.4", "4.5", "4.6", "4.9"}
TABLE_WIDTHS = {
    "4.1": [1.50, 1.60, 3.15],
    "4.2": [1.60, 2.00, 2.65],
    "4.3": [1.35, 1.55, 1.55, 1.80],
    "4.4": [1.35, 1.15, 1.00, 1.20, 5.20],
    "4.5": [0.70, 2.40, 1.10, 5.70],
    "4.6": [0.50, 2.20, 1.15, 1.15, 1.60, 3.30],
    "4.7": [1.55, 1.55, 3.15],
    "4.8": [1.55, 1.35, 3.35],
    "4.9": [2.50, 0.80, 1.00, 1.00, 1.00, 1.00, 2.60],
    "4.10": [2.00, 2.10, 2.15],
    "4.11": [1.80, 1.70, 2.75],
    "4.12": [2.00, 2.60, 1.65],
    "4.13": [1.70, 2.00, 2.55],
...
def add_cover(document):
    ...
    entries = [
        ...
        ("CHAPTER FOUR: SYSTEM IMPLEMENTATION, TESTING, RESULTS AND DISCUSSION", 13, True, 16),
        ("BY", 12, True, 14),
...
def parse_source(document, lines, pending_svg_links):
    ...
    while index < len(lines):
        ...
        if line.startswith("# ") and not skipped_title:
            skipped_title = True
            continue
        if line == "## CHAPTER FOUR":
            add_chapter_title(document, "CHAPTER FOUR", size=14)
            continue
        if line == "## SYSTEM IMPLEMENTATION, TESTING, RESULTS AND DISCUSSION":
            add_chapter_title(document, "SYSTEM IMPLEMENTATION, TESTING, RESULTS AND DISCUSSION", size=14, space_after=12)
            continue
        if line == "## REFERENCES":
            document.add_page_break()
        ...
        table_start = re.match(r"^\[\[TABLE\s+(4\.\d+)\s+START\]\]$", line)
        if table_start:
        ...
        figure_match = re.match(r"^\[\[FIGURE\s+4\.\d+:\s+(.+)\]\]$", line)
        if figure_match:
        ...
        caption_match = re.match(r"^\*\*Figure\s+(4\.\d+):\*\*\s*(.*)$", line)
        if caption_match:
...
def set_document_properties(document):
    ...
    properties.title = "Chapter Four — Phishing Digital Medium Detection System Using Machine Learning"
    properties.subject = "Final-Year Project Report — System Implementation, Testing, Results and Discussion"
    properties.author = "Honour Jesutofaye Jesutofaye"
    properties.keywords = (
        "phishing, smishing, implementation, testing, evaluation, Django, TF-IDF, "
        "Logistic Regression, hybrid detection, explainability, PhishGuard AI"
    )
    properties.comments = (
        "Implementation-grounded Chapter Four. SMS classifier results are reproduced from "
        "repository evidence; email and user-study results remain explicit placeholders."
    )
...
def inject_svg_fallbacks(docx_path, links):
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        for sequence, (png_rid, svg_source) in enumerate(links, start=1):
            svg_name = f"phishguard-ch4-{sequence}.svg"
